...
import { ArrowLeft, FileText, Image, Presentation, Send, Bot, Lock, CheckCircle2, Circle } from 'lucide-react';
import { useApp } from '@/contexts/AppContext';

const mockMaterials = [
  { id: 'm1', name: 'Lecture Slides Week 7.pdf', type: 'pdf', icon: Presentation },
...
export default function SubjectDetail() {
  ...
  if (!course) return <div className="p-6 text-muted-foreground">Subject not found</div>;
  ...
  const sendChat = () => {
    ...
    setTimeout(() => {
      setMessages(prev => [
        ...prev,
        { role: 'ai', text: subjectAIResponses[Math.floor(Math.random() * subjectAIResponses.length)] },
      ]);
    }, 1000);
  };
  ...
  return (
    <div className="px-5 pt-6">
      <button onClick={() => navigate('/subjects')} className="flex items-center gap-1 text-sm text-muted-foreground mb-4 hover:text-foreground transition-colors">
        <ArrowLeft className="w-4 h-4" /> Back
      </button>

      <h1 className="text-2xl font-bold font-display text-foreground">{course.name}</h1>
      <p className="text-muted-foreground text-sm mb-6">{course.code}</p>

      {/* Materials */}
      <section className="mb-6">
        <h2 className="text-sm font-semibold text-foreground uppercase tracking-wider mb-3">Materials</h2>
        <div className="space-y-2">
          ...
          <p className="text-[10px] text-muted-foreground text-center mt-2">📌 Mock materials — not real files</p>
        </div>
      </section>

      {/* Assignments */}
      <section className="mb-6">
        <h2 className="text-sm font-semibold text-foreground uppercase tracking-wider mb-3">Assignments</h2>
        <div className="space-y-2">
          {subjectAssignments.map(a => (
            ...
            >
              ...
              <div className="flex-1">
                <p className={`text-sm font-medium ${a.completed ? 'line-through text-muted-foreground' : 'text-foreground'}`}>{a.name}</p>
                <p className="text-xs text-muted-foreground">Due: {a.deadline} · {a.difficulty}</p>
              </div>
            </button>
          ))}
        </div>
      ...
      <section className="mb-8">
        <div className="flex items-center gap-2 mb-3">
          <h2 className="text-sm font-semibold text-foreground uppercase tracking-wider">Subject AI</h2>
        ...
        <div className="bg-card border border-border rounded-2xl p-4 min-h-[160px] flex flex-col">
          <div className="flex-1 space-y-3 mb-3">
            {messages.length === 0 && (
              <div className="text-center py-4">
                ...
                <p className="text-xs text-muted-foreground">Ask about {course.name} materials</p>
              </div>
            )}
            {messages.map((m, i) => (
              <motion.div
          ...
          <div className="flex items-center gap-2 border-t border-border pt-3">
            ...
            />
            <button onClick={sendChat} disabled={!chatInput.trim()} className="p-1.5 text-primary disabled:text-muted-foreground">
